...
fn test_p192_reduce_mul() {
    let expected: P192Element = P192Element {
        x: [0x1d971573098a6847, 0xa6548a41ca47a71f, 0x8a4ab71f6cc7e431],
    };
    let n: P192MulResult = P192MulResult {
        x: [
            0x853922c6d05a85d8,
            0xac9ec312b6d991b2,
            0x056a05340cb1b062,
...
fn test_p192_add() {
    ...
    let n: P192Element = P192Element {
        x: [0x2d2b1be584243c86, 0x66ce8e3ae5ab02e4, 0x9dba9c2dc998e2b8],
    };
    let m: P192Element = P192Element {
...
fn test_p192_add_overflow() {
    ...
    let actual = P192_A + P192_A;
    assert_eq!(actual, expected);
}

#[test]
fn test_p192_sub() {
    let expected1: P192Element = P192Element {
        x: [0x7e3c52c04b6f31e2, 0xc266a5b17cba31b0, 0x78606659f6705e81],
    };
    let expected2: P192Element = P192Element {
...
fn test_p192_sub_overflow() {
    let expected: P192Element = P192Element { x: [0x3, 0x0, 0x0] };
    let n = P192Element { x: [0x0, 0x0, 0x0] };
    let actual = n - P192_A;
    assert_eq!(actual, expected);
}
...
fn test_p192_mul_overflow() {
    let expected: P192Element = P192Element { x: [0x2, 0x0, 0x0] };
    let n: P192Element = P192Element {
...
fn test_p192_sqr_overflow() {
    let expected: P192Element = P192Element { x: [0x9, 0x0, 0x0] };
    let actual = P192_A.sqr();
    assert_eq!(actual, expected);
}

#[test]
fn test_p192_pt_normalize() {
    let expected: P192Point = P192Point {
        x: P192Element {
...
fn test_p192_pt_double() {
    ...
    let actual = P192_G.double().normalize();
    assert_eq!(actual, expected);
}
